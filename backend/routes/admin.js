...
const { isSqlite, isPostgres } = require('../utils/dbDriver');
...
router.patch('/demandes-orientation/:id', [
  ...
  try {
    ...
    let finalSql;
    if (isSqlite()) {
      finalSql = `UPDATE demandes_orientation SET ${sets.join(', ')}, updated_at = datetime('now') WHERE id = ?`;
    } else if (isPostgres()) {
      finalSql = `UPDATE demandes_orientation SET ${sets.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`;
    } else {
