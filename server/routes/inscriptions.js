...
const { notifyNewInscription } = require('../../includes/notifications');
...
const validations = [
  ...
  body('contact').optional().trim().isLength({ max: 120 }),
  body('contact_telephone').optional().trim().isLength({ max: 20 }),
  body().custom((value) => {
    const allowed = [
